...
use crate::cluster_controller::poly_join_set::JoinSet;
use crate::cluster_controller::worker_client::{Rpc, WorkerClientErr};
...
impl WorkerRegistryHandle {
    pub(crate) async fn send(&self, addr: &GrpcAddr, rpc: Rpc) -> Result<(), WorkerCommunicationError> {
    ...
    pub(crate) async fn broadcast<I, Rsp>(&self, requests: I) -> Vec<Result<Rsp, WorkerCommunicationError>>
    where
        I: IntoIterator<
            Item = (
                GrpcAddr,
